...
public class CRCVerify {
	  ...
	  private static final short[] CRC_CCITT_2=
	  { /* CRC-CCITT=X16+X12+X5+1 余式表 */
	  ...
	  public static String CRC16_Byte(byte[] cmd, int offset, int length)
	  {// 字节查表/Byte remainder table
	    if (cmd == null || cmd.length < 1)
	      return null;

	    int i = offset;  // data offset
	    int cmd_length = offset + length;
	    short crc = 0;
	    for (; i < cmd_length; i++) {
	      crc =(short)(crc<<8 ^ CRC_CCITT_2[((crc >> 8) ^cmd[i]) & 0xFF]);
	    }
	    int tmp_num = crc;
	    byte[] crcBytes = new byte[2];
	    crcBytes[1] = (byte)(tmp_num & 0xFF);
	    tmp_num = tmp_num >> 8;
	    crcBytes[0] = (byte)(tmp_num & 0xFF);
	    
	    return ConvertTools.bytesToHexString(crcBytes);
	  }
}
