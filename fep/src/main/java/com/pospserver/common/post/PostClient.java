...
import com.pospserver.common.minafilter.ClientCodecFactory;
...
public class PostClient implements IPostClient {
	...
	public boolean createConnection(MessageMina messageMina) {
		...
		try {
			...
			ip = sysParaService.getParaValue("FEP", "KMS_IP");// pos连接ip
			...
			port = Integer.parseInt(sysParaService.getParaValue("FEP", "KMS_PORT"));// pos连接端口
			...
			ClientCodecFactory clientcode = new ClientCodecFactory(4);
			...
			if (cf == null) {
				...
				logger.info("**** socket cann't connection !****");
			...
			if (session != null) {
				...
				logger.info("**** session cann't create !****");
			...
			if (readFuture.awaitUninterruptibly(
				...
				result = false;
				if (session != null) {
					logger.info("**** session close start !****");
					session.close(true);
				}
				if (connector != null) {
...
//					session.close(true);
			...
			logger.info("**** connector isDisposed=" + connector.isDisposed());

			if (connector != null) {
				
				connector.dispose();
				logger.info("**** connector.dispose isDisposed=" + connector.isDisposed());
